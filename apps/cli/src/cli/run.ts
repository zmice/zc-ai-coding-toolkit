...
export async function executeRun(
  ...
  if (!adapter) {
    ...
    console.error(
      `[zc run] 错误：未知的 CLI 适配器 "${options.cli}"。可用适配器：${available.join(", ")}`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (!detected) {
    console.error(
      `[zc run] 错误：未检测到 "${options.cli}" CLI 或它当前不可用。请先安装后重试。`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  console.log(`[zc run] 已启动 ${options.cli} 工人（PID: ${worker.pid}）`);
