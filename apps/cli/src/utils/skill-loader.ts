 ...
 * Root = repo root; this file lives under apps/cli/src/utils/.
 */
...
function resolveToolkitContentRoot(rootDir?: string): string {
  return join(resolveRoot(rootDir), "packages", "toolkit", "src", "content");
}
...
// Scan packages/toolkit/src/content/skills/*/body.md for all skills.
export async function discoverSkills(rootDir?: string): Promise<SkillMeta[]> {
  const skillsDir = join(resolveToolkitContentRoot(rootDir), "skills");
  ...
  for (const entry of entries) {
    ...
    const skillPath = join(skillsDir, entry.name, "body.md");
    try {
...
// Scan packages/toolkit/src/content/commands/*/body.md for all commands.
export async function discoverCommands(rootDir?: string): Promise<CommandMeta[]> {
  const commandsDir = join(resolveToolkitContentRoot(rootDir), "commands");
  ...
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const cmdPath = join(commandsDir, entry.name, "body.md");
    try {
...
// Scan packages/toolkit/src/content/agents/*/body.md for all agent definitions.
export async function discoverAgents(rootDir?: string): Promise<AgentMeta[]> {
  const agentsDir = join(resolveToolkitContentRoot(rootDir), "agents");
  ...
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const agentPath = join(agentsDir, entry.name, "body.md");
    try {
